...
public:
	...
	node_pointer copy_node(node_pointer to_copy)
	{
		...
		_node_alloc.construct(node, node_type(new_val, nullptr, _sentinal, _sentinal, false, to_copy->_color, to_copy->_count));
	...
	void leftRotate(node_pointer centre) {
		node_pointer y = centre->_right;
		centre->_right = y->_left;
		if (y->_left != _sentinal) {
			y->_left->_parent = centre;
		}
		y->_parent = centre->_parent;
		if (centre->_parent->_is_sentinal) {
			this->_root = y;
		} else if (centre == centre->_parent->_left) {
			centre->_parent->_left = y;
		} else {
			centre->_parent->_right = y;
		}
		y->_left = centre;
		centre->_parent = y;
		centre->_count = centre->_right->_count + centre->_left->_count + 1;
		y->_count = y->_right->_count + y->_left->_count + 1;
	}

	// rotate _right at node x
	void rightRotate(node_pointer centre) {
		node_pointer y = centre->_left;
		centre->_left = y->_right;
		if (y->_right != _sentinal) {
			y->_right->_parent = centre;
		}
		y->_parent = centre->_parent;
		if (centre->_parent->_is_sentinal) {
			this->_root = y;
		} else if (centre == centre->_parent->_right) {
			centre->_parent->_right = y;
		} else {
			centre->_parent->_left = y;
		}
	...
	size_t	search_node(node_pointer node) {
		if (node == nullptr) {
			return (0);
		}
		return (node->_index + search_node(node->left) + search_node(node->right));
	}
	...
	node_pointer insert(const value_type &val) {
		...
		new_node->_left = _sentinal;
		new_node->_right = _sentinal;
		new_node->_color = 1; // new node must be red
		new_node->_count = 1;
		node_pointer y = _sentinal;
		node_pointer x = _root;
		// std::cout << "root count" << _root->_count << std::endl;
		// std::cout << "root left count" << _root->_left->_count << std::endl;
		// std::cout << "root right count" << _root->_right->_count << std::endl;
		while (!x->_is_sentinal)
        {
			y = x;
			x->_count++;
			if (_comp(*new_node->_data, *x->_data))
            {
				x = x->_left;
			}
            else
		...
		if (y->_is_sentinal) {
			...
			_sentinal->_right = _root;
			_sentinal->_left = _root;
		} else if (_comp(*new_node->_data, *y->_data)) {
			y->_left = new_node;
		} else {
			y->_right = new_node;
		}
		...
		_size++;
		if (new_node->_parent->_is_sentinal){
			new_node->_color = 0;
			return (new_node);
		}

		// if the grandparent is sentinal, simply return
		if (new_node->_parent->_parent->_is_sentinal) {
			return (new_node);
		}
	...
	node_pointer findByIndex(node_pointer node, size_t index) {
		if (!node)
			return nullptr;
		
		size_t leftCount = (node->_left) ? node->_left->_count : 0;
        if (index < leftCount)
            return findByIndex(node->_left, index);
        else if (index == leftCount)
            return node;
        else
            return findByIndex(node->_right, index - leftCount - 1);
	}
	...
	void deleteNodeByIndex(const size_t index)
	{
		node_pointer val = findByIndex(_root, index);
		if (!val->_is_sentinal)
			deleteNodeHelper(val);
	}
